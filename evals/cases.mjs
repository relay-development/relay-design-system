...
export const CASES = [
  ...
  },
  {
    ...
    // 初回実行でエージェントが filter-chip（即時反映用）を選んだため、送信を伴うシナリオである
    // ことを「URL 共有」の要件で意図レベルに翻訳した（コンポーネント名は依然出さない）
    id: "listing-filter",
    prompt:
  ...
  },
  {
    ...
    rubric: [
      ...
      "場面のイラスト（挿絵）にはブランドアセット（list_assets）を使い、独自の SVG イラストを描いていない。チェックマーク等の小さな UI アイコンの inline SVG は違反ではない（審査対象は挿絵・イラストのみ）",
