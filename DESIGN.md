...
Multi Selector     : <div class="multi-selector multi-selector-md"><button class="multi-selector-trigger" aria-labelledby="項目名 値" aria-haspopup="dialog" aria-expanded="false" popovertarget="p"><span class="multi-selector-value" data-placeholder="選択してください">名前、メール</span></button><div class="multi-selector-panel" id="p" popover role="dialog"><label class="checkbox-label multi-selector-option"><input type="checkbox" class="checkbox">名前</label>…</div></div>  ← 決まった選択肢（〜15 個）から複数を選ぶ折りたたみ。中は普通の checkbox（listbox にしない）。場所があるなら checkbox を並べる、1 つなら selector
