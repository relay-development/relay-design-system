...
Token Input        : <div class="token-input"><div class="token-input-field"><input class="input input-md">…<button class="btn btn-md btn-neutral btn-outline">追加</button></div><ul class="token-input-list" aria-label="追加した担当者"><li class="token-input-item"><span class="token-input-label">田口</span><button class="token-input-remove" aria-label="『田口』を削除">×</button></li></ul></div>  ← 複数の値を 1 つずつ追加・削除する入力欄。トークンは入力欄の下（中に入れない）。追加は Enter か「追加」で確定、追加・削除は aria-live で告知、削除後は次のトークンか入力欄へフォーカス
