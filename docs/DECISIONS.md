...
## トースト／スナックバーは作らない — フィードバックの行き先を Primer 準拠で 4 つに分ける（2026-09）

- **背景**: 利用側から「ユーザー操作へのシステムフィードバックとしてスナックバー／トーストを追加したい」と要望があった。DS には alert（ページ内常設）と modal（最前面）しかなく、alert ヘッダに「トースト代わりに使わない」と書いてあるだけで、操作直近に置く結果表示の行き先が無かった。GitHub Primer は Toast をコンポーネントとして持たず、アクセシビリティガイドで「significant accessibility concerns and are not recommended」と非推奨にしている（時間制限 WCAG 2.2.1、発生場所と DOM 位置の乖離 1.3.2、フォーカス管理、拡大表示で見えない、自動消去で見逃す）。代替は「単純な成功は出さない / 操作直近は InlineMessage / ページ級は Banner / 注意を要するエラーは Dialog」
- **判断**: Primer と同じ設計を採る。トースト／スナックバーは作らず、行き先を 4 つに固定する — (1) 結果が画面で自明なら出さない、(2) 操作した場所の直近（ボタン横・フォーム直下・行内）= 新設の `inline-message`、(3) ページ全体への常設のお知らせ = `alert`（Banner 相当）、(4) 操作を止めて判断を求めるエラー = `modal`（Dialog 相当）。inline-message はアイコン＋ 1 行の文言だけで枠を持たず、theme は Primer に合わせて success / negative / warning / neutral（取得・利用できない）の 4 つで info を持たない（お知らせは alert-info）。時間での自動消去は禁止し、消えるのは次の操作か明示的な操作のみ。入力項目 1 つの検証は既存の `field-error-text` のまま（Primer の FormControl.Validation と InlineMessage が別物なのと同じ）
- **同時に**: カタログにガイドページ「システムフィードバックのアクセシビリティ」（feedback.html、ガイドライン群）と docs/ACCESSIBILITY.md 8 章「パターン別ガイド: システムフィードバック」を追加し、トーストを意図的に用意しない理由を WCAG 基準と対応させて明文化した（get_accessibility の topic "トースト" / "システムフィードバック" で到達する）。inline-message のウェイトは bold（本文の中で結果の一文が埋もれないよう、link と同じ判断）
