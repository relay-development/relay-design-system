...
## agent eval をルール変更 PR の CI ゲートにする（2026-09）

evals の Phase 4。2026-09-02〜04 の集中改善（search・select・ヘッダ補強・ソース検出・出力隔離）で
regression 8 お題が 3 回連続 全 PASS になり、失敗の原因が「生成ブレ」より「知識の穴 / 採点基準の誤り」
に寄ったため、PR 上で回帰を止める価値がコストを上回ると判断した。

- **対象を paths で絞る**: エージェントが読む知識（MCP 実装・CSS ヘッダ・トークン・DESIGN.md・a11y 文書・
  スニペット・アセット・index ビルド）とお題/ランナー自身。それ以外の PR では LLM を消費しない
- **regression のみ**: capability は改善メーターであり、ゲートに使うと改善途中の変更を止めてしまう。
  週次の手動フル実行で見る
- **1 trial**: `--trials 2`（pass^2）はコスト 2 倍で偽陰性も増える。✓→✗ は Re-run で再確認し、2 回続けば劣化
- **結果は PR コメント 1 つを更新**: push ごとに増やさない。生成物・行動ログは Artifact に 30 日保存し、
  切り分け（知識 / 採点基準 / お題）はこれまでどおり人が行う
- **認証は API キー**: CI ではサブスク枠でなく `ANTHROPIC_API_KEY`。モデルは Variable で差し替え可能
- **run.mjs**: `--kind` を追加。`resolveClaudeBin` の zsh 依存を外し Linux でも動くようにした
