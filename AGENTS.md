# relay Design System — Codex 用ガイド

Codex が本リポジトリで作業する際に毎回読み込む運用ガイド。
...
- MCP（`src/mcp/`）・DESIGN.md・コンポーネントヘッダを変更したら `npm run eval` で前後のエージェント挙動を測定する（LLM 生成が走るため実行はユーザーに確認してから。詳細は [evals/README.md](evals/README.md)）
